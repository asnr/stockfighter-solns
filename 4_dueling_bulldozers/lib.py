...
class StockPurse:
    def __init__(self, venue, stock, account, stocks_held=0, basis=0):
        self._session = APISession()
        self._venue = venue
        self._stock = stock
        self._account = account
        self._basis = basis
        self._stocks_held = stocks_held
    ...
    def qty_filled(self, id):
        # order important, I don't think I deleted the
        # closed asks/bids
        if id in self._closed_asks:
            order = self._closed_asks[id]
        elif id in self._closed_bids:
            order = self._closed_bids[id]
        elif id in self._open_asks:
            order = self._open_asks[id]
        elif id in self._open_bids:
            order = self._open_bids[id]
        else:
            order = None

        return order['totalFilled']
    ...
    def buy(self, type, qty, price):
        resp = self._session.buy(self._venue, self._stock, self._account,
                                 type, qty, price)
    ...
    def sell(self, type, qty, price):
        resp = self._session.sell(self._venue, self._stock, self._account,
                                  type, qty, price)
    ...
    def cancel(self, id):
        ...
        resp = self._session.cancel_order(self._venue, self._stock, id)
